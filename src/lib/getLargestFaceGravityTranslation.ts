...
export default (
  ...
  if (face) {
    ...
    const xMin = Math.min(0, width - state.width);
    const xMax = Math.max(0, state.width - width);
    const xDelta = faceCenterX - centerX;
    const yMin = Math.min(0, height - state.height);
    const yMax = Math.max(0, state.height - height);
